    ...
    namespace CfCRM.DATN.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class DishOrderStatusController : ControllerBase
        {
            IDishOrderStatusService service;
            public DishOrderStatusController(IDishOrderStatusService _service)
            {
                service = _service;
            }
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]DishOrderStatus model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]DishOrderStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]DishOrderStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]DishOrderStatus model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountDishOrderStatus()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] DishOrderStatusDTParameters parameters)
            {
                try
                {
