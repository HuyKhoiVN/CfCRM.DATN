...
namespace CfCRM.DATN.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class DishOrderDetailController : ControllerBase
    {
        IDishOrderDetailService service;
        public DishOrderDetailController(IDishOrderDetailService _service)
        {
            service = _service;
        }
        ...
        [HttpGet]
        [Route("api/ListByOrderId/{id}")]
        public async Task<IActionResult> ListByOrderId(int id)
        {
            try
            {
                var dataList = await service.ListByOrderId(id);
                if (dataList == null || dataList.Count == 0)
                {
                    return NotFound();
                }
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] DishOrderDetail model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody] DishOrderDetail model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] DishOrderDetail model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] DishOrderDetail model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        [HttpGet]
        [Route("api/Count")]
        public int CountDishOrderDetail()
        {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] DishOrderDetailDTParameters parameters)
        {
            try
            {
