    ...
    namespace CfCRM.DATN.Controllers
    {
        ...
        {
            IPurchaseOrderDetailService service;
            public PurchaseOrderDetailController(IPurchaseOrderDetailService _service)
            {
                service = _service;
            }
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]PurchaseOrderDetail model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]PurchaseOrderDetail model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]PurchaseOrderDetail model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]PurchaseOrderDetail model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountPurchaseOrderDetail()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] PurchaseOrderDetailDTParameters parameters)
            {
                try
                {
