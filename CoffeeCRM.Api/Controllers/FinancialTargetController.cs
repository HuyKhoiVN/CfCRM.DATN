    ...
    namespace CfCRM.DATN.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class FinancialTargetController : ControllerBase
        {
            IFinancialTargetService service;
            public FinancialTargetController(IFinancialTargetService _service)
            {
                service = _service;
            }
        ...
        public async Task<IActionResult> Detail(int? Id)
        {
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]FinancialTarget model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]FinancialTarget model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]FinancialTarget model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]FinancialTarget model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountFinancialTarget()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] FinancialTargetDTParameters parameters)
            {
                try
                {
