    ...
    namespace CfCRM.DATN.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class InventoryDiscrepancyController : ControllerBase
        {
            IInventoryDiscrepancyService service;
            public InventoryDiscrepancyController(IInventoryDiscrepancyService _service)
            {
                service = _service;
            }
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]InventoryDiscrepancy model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]InventoryDiscrepancy model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]InventoryDiscrepancy model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]InventoryDiscrepancy model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountInventoryDiscrepancy()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] InventoryDiscrepancyDTParameters parameters)
            {
                try
                {
