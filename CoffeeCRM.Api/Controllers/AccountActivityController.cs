...
namespace CfCRM.DATN.Controllers
{
        ...
        {
            IAccountActivityService service;
            public AccountActivityController(IAccountActivityService _service)
            {
                service = _service;
            }
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]AccountActivity model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]AccountActivity model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]AccountActivity model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]AccountActivity model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountAccountActivity()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] AccountActivityDTParameters parameters)
            {
                try
                {
