...
namespace CfNCKH.Core.BaseCore
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        public UnitOfWork(CfCRMNCKHv02Context dbContext, IHttpContextAccessor accessor, ILogger<UnitOfWork> logger)
        {
        ...
        /// </summary>
        public CfCRMNCKHv02Context DataContextRead { get; }

        /// <summary>
        /// Define a property of context write class
        /// </summary>
        public CfCRMNCKHv02Context DataContextWrite { get; }
        ...
        /// <param name="entity"></param>
        /// <param name="saveChange"></param>
        public void Delete<TEntity>(params int[] ids) where TEntity : class
        {
            try
            {
        ...
        private void GenerateBaseFieldInsert<TEntity>(params TEntity[] entities)
        {
            foreach (var entity in entities)
            {
                ...
                if (properties == null)
                {
                    properties = entity.GetType().GetProperty("CreateDate");
                }
                if (properties != null)
                {
                ...
                {
                    var propertiesVal = properties.GetValue(entity);
                    if (properties.PropertyType == typeof(bool) && propertiesVal == null)
                    {
        ...
        //            if (idProperty.PropertyType == typeof(Guid) && id == null)
        //            {
    ...
    {
        public static void DetachLocal<TEntity>(this CfCRMNCKHv02Context context, TEntity entity) where TEntity : class
        {
            var entityType = typeof(TEntity);
            var idProperty = entityType.GetProperty("Id");

            if (idProperty == null)
                throw new InvalidOperationException($"Entity {entityType.Name} does not have a property named 'Id'.");

            int id = (int)idProperty.GetValue(entity);

            var local = context.Set<TEntity>().Local
                .FirstOrDefault(entry => idProperty.GetValue(entry).Equals(id));
        ...
        public static bool EqualsId<TFirst, TSecond>(this TFirst one, TSecond two)
        {
            ...
            var firstIdProperty = firstType.GetProperty("Id");
            var secondIdProperty = secondType.GetProperty("Id");

            if (firstIdProperty == null || secondIdProperty == null)
                return false;

            var firstId = firstIdProperty.GetValue(one);
            var secondId = secondIdProperty.GetValue(two);
