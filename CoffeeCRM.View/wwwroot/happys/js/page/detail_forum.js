...
/*        console.error(error);*/
    ...
    setTimeout(function () {
        ...
        var imgArray = Array.from(imgCollection);
        imgArray.forEach(function (item) {
            item.src = item.src;
            item.onerror = function (e) {
                item.src = '/assets/media/images/Blog/NoImage.png';
            };
        });
    }, 1500);
})
...
async function loadDetailForum() {
    ...
    //var dataSuccess = await httpService.getAsync("forumpost/api/AddCountViewForumPost/" + forumId);
    var data = dataSourse.data[0];
    var img = data.forumPostAuthorPhoto ? data.forumPostAuthorPhoto : "/images/default/NoImage.png";
    var content = `
        <div class="row">
            <div class="col-lg-12 ">
                <div class="forum-post-top-header">
                    ...
                    <div class="top-post-author d-flex flex-column">
                        ...
                        <div class="forum-author-extra" >
                            <div>
                                ...
                                <a class="author-name" href="${(data.forumPostTypeId == systemConstant.notification_status_read ? `/thong-tin-ca-nhan/` + data.forumAccountUserName + `` : '')}">${data.forumPostTypeId == systemConstant.forum_post_type_incognito_id ? `Ẩn Danh` : data.forumPostAuthorName}</a>
                                ${data.forumPostStatusId == systemConstant.forum_post_type_waitting_id ? `<span style="color:#fb5401;">Bài viết chờ duyệt</span>` : ``}
                            </div>
                            <span>`+ moment(data.publishedTime).fromNow() + `</span>
                        </div>
                </div>
            </div>
            </div>
...
async function editItem(id) {
    ...
    $("html, body").animate({ scrollTop: postContentOffsetTop - 300 }, "slow");
