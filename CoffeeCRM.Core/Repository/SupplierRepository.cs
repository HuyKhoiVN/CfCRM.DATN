    ...
    public class SupplierRepository : ISupplierRepository
    {
        ...
        public async Task<DTResult<SupplierDto>> ListServerSide(SupplierDTParameters parameters)
        {
            ...
            var query2 = from s in query
                         where s.row.Active
                         join d in db.Debts on s.row.Id equals d.SupplierId into gj
                         from subDebt in gj.DefaultIfEmpty()
                         group subDebt by new
                         {
                             s.row.Id,
                             s.row.SupplierCode,
                             s.row.ContactInfo,
                         ...
                         select new SupplierDto
                         {
                             ...
                             Active = g.Key.Active,
                             TotalDebt = g.Sum(x => x != null && x.Active && !x.IsPaId ? x.TotalMoney : 0),
                             PaidAmount = g.Sum(x => x != null && x.Active && x.IsPaId ? x.TotalMoney : 0),
                             UnpaidCount = g.Count(x => x != null && x.Active && !x.IsPaId)
                         };


            //4. Sort
            query2 = query2.OrderByDynamic<SupplierDto>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<SupplierDto>()
            {
