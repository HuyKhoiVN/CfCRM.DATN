...
namespace CoffeeCRM.Core.Repository
{
    public class CashFlowRepository : ICashFlowRepository
    {
        ...
        public async Task<DTResult<CashFlowBaseDto>> ListServerSide(CashFlowDTParameters parameters)
        {
            ...
            //3.Query second
            var query2 = query.Select(c => new CashFlowBaseDto()
            {
                Id = c.row.Id,
                ...
                AccountName = db.Accounts.Where(a => a.Id == c.row.AccountId).Select(a => a.FullName).FirstOrDefault() ?? string.Empty

            });
            //4. Sort
            query2 = query2.OrderByDynamic<CashFlowBaseDto>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<CashFlowBaseDto>()
            {
