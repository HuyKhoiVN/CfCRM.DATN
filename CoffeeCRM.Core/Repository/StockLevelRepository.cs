...
case "lastUpdatedTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new StockLevel()
                {
                    Id = c.row.Id,
...
LastUpdatedTime = c.row.LastUpdatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<StockLevel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<StockLevel>()
                {
